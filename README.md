...
```bash
snakemake --use-conda --use-singularity --jobs 4 --configfile config.yaml --cluster-config cluster-config.yaml --cluster "sbatch --partition={cluster.partition} {cluster.gres} --mem={cluster.mem} --cpus-per-task={cluster.cpus} --time={cluster.time}" --conda-frontend conda --singularity-args "-B /ifs/data/research/unique"
```
...
```bash
snakemake --use-conda --use-singularity --jobs 4 --configfile config.yaml --cluster-config cluster-config.yaml --cluster "sbatch --partition={cluster.partition} {cluster.gres} --mem={cluster.mem} --cpus-per-task={cluster.cpus} --time={cluster.time}" --conda-frontend conda --singularity-args "-B /ifs/data/research/unique" --rerun-incomplete
```
